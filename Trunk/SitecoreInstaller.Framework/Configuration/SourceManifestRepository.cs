...
namespace SitecoreInstaller.Framework.Configuration
{
    public class SourceManifestRepository
    {
        ...
        public void Init()
        {
            ...
            sourcesRepository.Load(SourceFileName);
            var sources = sourcesRepository.GetElements("source").Select(source => new SourceManifest(source.Attribute("name").Value, source.Attribute("type").Value, source.Attribute("parameters").Value));
